      ...
      <div className="mx-auto w-full max-w-[1180px] bg-[#f1ecdf] px-3.5 pt-0 pb-8 font-sarabun md:px-4">
        <div className="anim-fade" style={animStyle(0)}>
          <SafetyCultureHero
            ...
            title={
              <>
                ทีมไหน <span className="text-[#F5BB00]">ปลอดภัยสุด</span>
              </>
            }
        ...
        <Card
          className="relative mb-5 flex items-center gap-4 overflow-hidden rounded-[24px] p-5 text-white shadow-[0_10px_25px_rgba(0,0,0,0.12)] anim-fade md:p-6"
          style={{ background: "linear-gradient(135deg, #3B210B, #5C3214)", ...animStyle(0.1) }}
        >
          ...
          <div className="relative flex w-full items-center gap-4">
            <div
              className="flex h-[60px] w-[60px] flex-shrink-0 items-center justify-center rounded-full border-[3px] border-white text-3xl shadow-[0_0_16px_rgba(245,187,0,0.35)] md:h-[68px] md:w-[68px]"
              style={{ backgroundColor: leadingTeam?.color ?? "#F5BB00" }}
            >
              🏆
            </div>
            <div className="min-w-0 flex-1">
              <span className="text-[12px] font-bold uppercase tracking-wider text-[#bcaaa4] md:text-[13px]">ทีมนำตอนนี้</span>
              <div className="mt-0.5 text-[26px] leading-none font-black tracking-tight text-white md:text-[30px]">{leadingTeam?.name ?? "-"}</div>
              <span className="mt-1.5 block text-[13.5px] font-black md:text-[14.5px]" style={{ color: leadingTeam?.color ?? "#F5BB00" }}>
                + {leadPoints.toLocaleString()} คะแนน เหนือกว่า {runnerUpTeam?.name ?? "-"}
              </span>
              {leadingTeam ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[11px] font-black text-[#ffe7b0]">
        ...
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[1.45fr_1fr]">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              {teamStandings.map((team, idx) => (
                <Card
                  key={team.id}
                  className="flex flex-col gap-2.5 rounded-[20px] border border-[#e5d6be] bg-[#FFFDF7] p-4 shadow-[0_6px_15px_rgba(62,36,13,0.02)] transition-all duration-200 hover:-translate-y-px hover:border-[#C49A45] hover:shadow-[0_8px_20px_rgba(62,36,13,0.05)] anim-fade"
                  style={animStyle(0.1 + idx * 0.04)}
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="min-w-[15px] text-[18px] font-black text-[#1A1A1A]">{team.rank}</span>
                      <div className="h-9 w-9 flex-shrink-0 rounded-[10px] border border-[#1A1A1A]" style={{ backgroundColor: team.color }} />
                    ...
                    <div className="flex-shrink-0 text-right">
                      ...
                      <span className="block text-[9px] font-bold text-[#8E8A81]">คะแนน</span>
                    </div>
                  </div>
                  <div className="h-[6px] w-full overflow-hidden rounded-full bg-[#EFEBE0]">
                    <div
                      className="h-full rounded-full transition-[width] duration-1000 ease-out"
                      style={{ width: mounted ? `${team.percent}%` : "0%", backgroundColor: team.color }}
                    />
                  </div>
                </Card>
              ))}
          ...
          <div className="flex flex-col gap-3">
            <Card
              className="flex flex-col gap-4 rounded-[20px] border border-[#e5d6be] bg-[#FFFDF7] p-5 shadow-[0_10px_24px_rgba(62,36,13,0.02)] anim-fade"
                ...
                {personalRankings.map((user) => (
                  <div
                    ...
                    className={cn(
                      ...
                      user.active
                        ? "border-[#F5BB00] bg-[#FFF9E6] shadow-[0_2px_8px_rgba(245,187,0,0.08)]"
                        : "border-transparent bg-transparent"
                    )}
                  >
                    <div className="flex min-w-0 items-center gap-2.5">
                      <span className={cn("min-w-[20px] font-black", user.active ? "text-[#F5BB00]" : "text-[#8E8A81]")}>{user.rank}</span>
                      <div className="min-w-0">
                        <span className="block truncate font-[750]">{user.name}</span>
                        <span className="block text-[11px] font-bold text-[#8E8A81]">{user.team}</span>
                      </div>
                    </div>
