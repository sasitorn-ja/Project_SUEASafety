...
function MobileConfiguredNode({
  ...
  if (children.length === 0) {
    const content = (
      <>
        {Icon && <Icon className="h-[14px] w-[14px] flex-shrink-0 text-[#0B82F0]" strokeWidth={2.35} />}
        <span className="truncate">{node.label}</span>
      </>
    );
    if (!node.href) {
      return <div className="flex min-h-9 items-center gap-2.5 rounded-lg px-2.5 text-[11px] font-bold text-[#55739B]">{content}</div>;
    }
    return (
      <NavTo
        href={node.href}
        onClick={closeDrawer}
        className={cn(
          "flex min-h-9 items-center gap-2.5 rounded-lg px-2.5 text-[11px] font-bold hover:bg-[#F5FAFF]",
          active ? "bg-[#F5FAFF] text-[#0B82F0]" : "text-[#55739B]"
        )}
      >
        {content}
      </NavTo>
  ...
  return (
    <div>
      ...
      {isOpen && (
        <div className="mt-1 ml-[18px] grid grid-cols-1 gap-1 border-l border-[#D7EAFE] pl-2">
          {children.map((child) => (
      ...
      >
        <div className="flex min-w-0 items-center gap-[9px]">
          <button
            ...
            className="inline-flex h-10 w-10 flex-shrink-0 cursor-pointer items-center justify-center rounded-full bg-transparent text-[#0B82F0] transition-opacity hover:opacity-70"
          ...
          <NavTo href="/" onClick={closeDrawer} className="flex min-w-0 items-center">
            <Image src="/images/brand/LOGO1_trim.png" alt="Safety Caring" width={210} height={36} priority className="h-[30px] w-auto object-contain" />
          </NavTo>
        </div>
      </div>
          ...
          <aside className="relative flex h-full w-[min(318px,86vw)] flex-col border-r border-[#D7EAFE] bg-white/98 shadow-[18px_0_34px_rgba(185,223,255,0.48)] backdrop-blur-[16px]" aria-label="เมนูหลัก">
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <div className="grid grid-cols-1 gap-2">
                ...
                {/* Profile card — clicking goes to /profile, submenu always visible */}
                <div>
                  <NavTo
                    href="/profile"
                    onClick={closeDrawer}
                    className={cn(
                      "flex min-h-11 w-full items-center gap-3 rounded-xl border border-[#D7EAFE] bg-[#F5FAFF] px-3 py-3 text-[#0B2F6B] transition-colors hover:bg-[#EEF7FF]",
                      profileSectionActive && "ring-1 ring-[#0B82F0]"
                    )}
                  >
                    <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center overflow-hidden rounded-full border border-[#D7EAFE] bg-white">
                      {displayImage && !profileImageFailed ? (
                        ...
                        <span className="text-[12px] font-black tracking-[0.08em] text-[#0B82F0]">{displayInitials}</span>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-[13px] font-black">{displayName}</div>
                      <div className="truncate text-[11px] font-bold text-[#55739B]">@{displayUsername}</div>
                    </div>
                  </NavTo>

                  <div className="mt-1 ml-[18px] grid grid-cols-1 gap-1 border-l border-[#D7EAFE] pl-2">
                    {PROFILE_SUBMENU_ITEMS.map((item) => {
                      ...
                      return (
                        <NavTo
                          ...
                          onClick={closeDrawer}
                          className={cn(
                            "flex min-h-9 items-center gap-2.5 rounded-lg px-2.5 text-[11.5px] font-bold transition-colors hover:bg-[#F5FAFF]",
                            isExactNavActive(pathname, item.href) ? "bg-[#F5FAFF] text-[#0B82F0]" : "text-[#55739B]"
                          )}
                        >
                          <Icon className="h-[15px] w-[15px] flex-shrink-0 text-[#0B82F0]" strokeWidth={2.35} />
                    ...
                    return (
                      <div key={item.id}>
                        <button
                          ...
                          aria-expanded={isOpen}
                          className={cn(
                            "flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-xs font-extrabold outline-none transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B82F0]",
                            adminActive ? "bg-[#F5FAFF] text-[#0B82F0]" : "bg-white text-[#55739B] hover:bg-[#F5FAFF]"
                          )}
                        >
                          <Icon className="h-4 w-4 flex-shrink-0 text-[#0B82F0]" strokeWidth={2.35} />
                        ...
                        {isOpen && (
                          <div className="mt-1 ml-[18px] grid grid-cols-1 gap-1 border-l border-[#D7EAFE] pl-2">
                            ...
                            {adminSections.length === 0 && (
                              <div className="px-2.5 py-3 text-[11px] font-bold text-[#55739B]">ยังไม่มีเมนูย่อย Admin</div>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    return (
                      <div key={item.id}>
                        <button
                          ...
                          className={cn(
                            "flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-xs font-extrabold outline-none transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B82F0]",
                            sectionActive ? "bg-[#F5FAFF] text-[#0B82F0]" : "bg-white text-[#55739B] hover:bg-[#F5FAFF]"
                          )}
                        >
                          <Icon className="h-4 w-4 flex-shrink-0 text-[#0B82F0]" strokeWidth={2.35} />
                          <span className="flex-1 truncate text-left">{item.label}</span>
                            ...
                            {item.children.map((sub) => {
                              ...
                              return (
                                <NavTo
                                  ...
                                  className={cn(
                                    "flex min-h-9 items-center gap-2.5 rounded-lg px-2.5 text-[11.5px] font-bold transition-colors hover:bg-[#F5FAFF]",
                                    isExactNavActive(pathname, sub.href) ? "bg-[#F5FAFF] text-[#0B82F0]" : "text-[#55739B]"
                                  )}
                                >
                                  <SubIcon className="h-[15px] w-[15px] flex-shrink-0 text-[#0B82F0]" strokeWidth={2.35} />
                  ...
                  return (
                    <NavTo
                      ...
                      className={cn(
                        ...
                        active && enabled ? "bg-[#0B82F0] text-white shadow-[0_8px_18px_rgba(11,130,240,0.20)]" : "bg-white text-[#55739B] hover:bg-[#F5FAFF]",
                        !enabled && "cursor-not-allowed opacity-60"
                      )}
                      aria-disabled={!enabled}
                    >
                      <Icon className={cn("h-4 w-4 flex-shrink-0", active && enabled ? "text-white" : "text-[#0B82F0]")} strokeWidth={2.35} />
                      <span className="truncate">{item.label}</span>
                    </NavTo>
                  );
                })}
